    ...
    public class TolemController : Controller
    {
        ...
        public async Task<IActionResult> Tolem(string user_id, string type, int pg_result, string pg_amount, DateTime pg_payment_date, string pg_user_phone, string ptype)
        {
            ...
            if (pg_result == 1)
            {
                ...
                if (ptype == "pan")
                {
                    await db.AddAsync(new Ofpay { IdentityId = user_id, type = type,Price = pg_amount.Split(".")[0], Time = DateTime.Now });
        ...
        {            
            ...
            if (command == "pay")
                ...
                if (qaccount.pan & pan)
                {
                    ...
                    if (pays.Count == 0)
                    {
                        await db.Ofpays.AddAsync(new Ofpay { Time = DateTime.Now, IdentityId = user.Id, type = qaccount.type, Price = String.Format("{0:0.##}", sum) });
        ...
        public async Task<IActionResult> Orderqiwi([FromBody] OrderQiwiViewModel body)
        {
            ...
            if (oldorder == null)
            {
                await db.Qiwipays.AddAsync(new Qiwipay { account = acount, txn_date = DateTime.Now, sum = body.price, type = body.type, number = body.num, prv_txn = prv, pan = body.pan, UserId = user.Id });
            }
            else
            {
                return Json("already");
        ...
        [HttpPost("checkqiwi")]
        public async Task<IActionResult> CheckQiwi([FromBody] CheckViewModel body)
        {
